Page({
      ...
      if (res.result && res.result.code === 0) {
        ...
        this.setData({
          product: {
            ...
            shipFrom: product.shipFrom || '上海',
            merchantId: product.merchantId || '',
            merchantName: product.merchantName || '优品服饰'
          },
          selectedColor: '',
          selectedSize: '',
          selectedSpec: '',
  ...
  contactService() {
    // 跳转到智能客服页面，携带商品信息
    const pid = this.data.product.productId || this.data.product._id || this.data.product.id;
    const productInfo = {
      _id: pid,
    ...
    wx.navigateTo({
      url: `/pages/chat/chat?productId=${pid}&productInfo=${encodeURIComponent(JSON.stringify(productInfo))}`
    });
  },
  ...
  contactMerchant() {
    // 跳转到与商家的聊天页面
    const pid = this.data.product.productId || this.data.product._id || this.data.product.id;
    const merchantName = this.data.product.merchantName || '优品服饰';
    const merchantId = this.data.product.merchantId || '97b16bdb69fde9ea018468c710a9a009';
    const productInfo = {
      _id: pid,
      name: this.data.product.name,
      price: this.data.product.price,
    ...
    wx.navigateTo({
      url: `/pages/chat/chat?productId=${pid}&productInfo=${encodeURIComponent(JSON.stringify(productInfo))}&chatType=merchant&merchantName=${encodeURIComponent(merchantName)}&merchantId=${merchantId}`
    });
  },
  ...
  async loadReviews(productId) {
    try {
      ...
      if (res.result && res.result.code === 0) {
        const reviews = (res.result.data || []).slice(0, 3).map(item => ({
          ...item,
          timeStr: item.createTime ? this.formatReviewTime(item.createTime) : ''
        }));
  ...
  viewAllReviews() {
    const pid = this.data.product.productId;
    wx.navigateTo({
      url: `/pages/review-list/review-list?productId=${pid}`
    });
  },
