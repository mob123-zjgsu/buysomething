...
Page({
  ...
  async loadAddress(id) {
    ...
    try {
      ...
      if (res.result && res.result.code === 0) {
        const addr = (res.result.data || []).find(a => a._id === id);
        if (addr) {
          // 设置省市区picker索引
          const provinceIdx = PROVINCES.indexOf(addr.province);
          const cities = provinceIdx >= 0 ? Object.keys(REGION_DATA[PROVINCES[provinceIdx]]) : [];
          const cityIdx = cities.indexOf(addr.city);
          const districts = cityIdx >= 0 ? REGION_DATA[PROVINCES[provinceIdx]][cities[cityIdx]] : [];
          const districtIdx = districts.indexOf(addr.district);
  ...
  async onSubmit() {
    ...
    try {
      ...
      const params = { action, ...this.data.formData };
      if (this.data.isEdit) {
        params.addressId = this.data.editId;
      }

      const res = await wx.cloud.callFunction({ name: 'address', data: params });
      wx.hideLoading();
